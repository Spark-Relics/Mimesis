...
当前实现补充（2026-09-09）：已有可保存、录制、编辑并执行的 JSON 采集流程，支持初始化动作、字段提取、翻页循环和参数替换，详见[实际操作指南](collection-workflows.md)。下文的任意 TypeScript/JavaScript 项目、发布/回滚体系和产品内 AI 修复仍为规划，旧草稿编辑入口已从工作区移除。
