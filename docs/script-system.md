...
当前实现补充（2026-09-09）：已有可保存、录制、编辑并执行的 JSON 采集流程，支持初始化动作、字段提取、翻页循环和参数替换，详见[实际操作指南](collection-workflows.md)。下文的任意 TypeScript/JavaScript 项目、脚本项目导入导出体系和产品内 AI 修复仍为规划（JSON 采集流程的不可变发布版本、内容摘要与回滚已实现，见[研发验收计划](engineering-plan.md)第五批），旧草稿编辑入口已从工作区移除。
