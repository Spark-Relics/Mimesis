...
版本：0.2 · 初稿日期：2026-09-06 · 状态：完整目标架构提案，部分已实现。当前研发范围、依赖顺序与完成标准以[研发验收计划](engineering-plan.md)为准。

实现进度（2026-09-09）：已有 Electron 桌面、Profile 隔离、真实点击/输入录制，以及每实例保存的 JSON 初始化与翻页采集流程。桌面和 HTTP 网关共用执行器，已验证字段提取、去重、参数化复用、两页外网采集与 JSON/CSV/NDJSON 导出。网关支持单执行器持久化排队、鉴权、幂等、取消和按实例归档，详见[实际接口与限制](gateway.md)和[录制与循环指南](collection-workflows.md)。下文仍是完整目标架构，不代表账号池、通用脚本沙盒、运行中 Agent 或高并发调度已经实现。
...
主要入口覆盖浏览与录制、流程编排、浏览器环境、运行记录及服务配置。当前采用顶部导航与单列工作区，浏览器、编排、结果按模式切换；禁止恢复源码与浏览器左右并排或固定侧栏布局。新增功能先设计完整状态与设计稿，再实现真实行为。

产品内 Agent 尚未接入。未来应通过单列审查流程展示输入、观察、修改建议、差异、试跑和发布结果；只在真实调用模型时展示模型状态。生成与修复不能静默替换已发布版本。

现有 `design/ui-workbench-v1.png` 保留为早期浏览器工作台视觉参考，其常驻 AI 执行面板不作为默认运行模式。本次调整更新规划，尚未重绘该图片。

当前已有 Electron 应用、录制与采集流程、Profile 隔离及本地网关，并有单元和 Electron 测试。其余规划不因出现在本文件中就代表已经实现，具体状态以研发验收计划为准。
