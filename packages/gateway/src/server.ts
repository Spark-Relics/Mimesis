...
export function gatewayConfigFromEnv(env: NodeJS.ProcessEnv): GatewayConfig | undefined {
  if (env.CLAWLER_GATEWAY_TOKEN === undefined && env.CLAWLER_GATEWAY_PORT === undefined) return undefined;
  return configSchema.parse({ token: env.CLAWLER_GATEWAY_TOKEN, port: Number(env.CLAWLER_GATEWAY_PORT ?? "17840") });
}
...
const statusCodes: Record<string, number> = {
  INVALID_INPUT: 400, FORBIDDEN: 403, NOT_FOUND: 404, BUSY: 409, CONFLICT: 409,
  RESULT_NOT_READY: 409, QUEUE_FULL: 429, UNAVAILABLE: 503, STORAGE_FAILED: 503,
};
...
async function readJson(request: IncomingMessage): Promise<unknown> {
  if (request.headers["content-type"]?.split(";")[0]?.trim().toLowerCase() !== "application/json" || request.headers["content-encoding"])
...
export class GatewayServer {
  private constructor(private readonly server: Server, public readonly port: number) {}

  static async listen(input: GatewayConfig, queue: GatewayQueue, listInstances: () => AutomationInstance[]): Promise<GatewayServer> {
    const config = configSchema.parse(input);
    const expected = createHash("sha256").update(`Bearer ${config.token}`).digest();
    const server = createServer({ maxHeaderSize: 16_384, requestTimeout: 15_000, headersTimeout: 10_000, keepAliveTimeout: 5000 }, (request, response) => {
      ...
      void (async () => {
        ...
        if (match) {
          ...
          if (request.method === "GET" && action === "result") {
            const format = z.enum(["json", "csv", "ndjson"]).parse(url.searchParams.get("format") ?? "json");
            const job = queue.get(id);
            if (job.status !== "succeeded" || !job.run?.result) throw new GatewayError("RESULT_NOT_READY");
            response.writeHead(200, { "Content-Type": resultContentTypes[format], "Content-Disposition": `attachment; filename="${id}.${format}"` });
            response.end(serializeResult(cleanResult(job.run.result, job.submission.cleaning), format));
            return;
          }
        }
        ...
        let code = toErrorCode(error);
        let status = statusCodes[code] ?? 500;
        let publicCode: string = code;
        if (error instanceof HttpError) { status = error.status; publicCode = error.code; }
        if (error instanceof GatewayError) { publicCode = error.code; status = statusCodes[publicCode] ?? 500; }
